...
def make_env(stacked=False):
    def _init():
        return RenkoFilterFastEnv(
            ...
            reward_config=None,
            mode='test',       # Test Mode
            split_ratio=0.8,   # Standard 80/20 Split for XAUUSD
...
def calculate_advanced_metrics(initial_cap, final_cap, trades_pnl, daily_returns, curve, days_count):
    ...
    # Kelly
    # Considering Breakeven: Expectancy > 0 implies Positive Kelly roughly
    # Let's just output Expectancy as it's cleaner
    
    # Formatted Report
    report = []
    report.append("==================================================")
    report.append("           QUANTITATIVE ANALYSIS REPORT           ")
    report.append("        WEIGHTED ENSEMBLE (XAUUSD BASE)           ")
    report.append("==================================================")
    report.append(f"Metric Used: XAUUSD Test Set (80/20 Split)")
...
def plot_heatmap(dates, daily_rets):
    ...
    plt.title('Monthly Returns Heatmap - Weighted Ensemble (XAUUSD)')
    plt.tight_layout()
    plt.savefig('RL_Agent_Final/heatmap_weighted_xauusd.png')
    print("Saved heatmap to RL_Agent_Final/heatmap_weighted_xauusd.png")
    plt.close()

def evaluate_weighted_ensemble_xauusd():
    print("Loading Models for Weighted Ensemble (XAUUSD Base)...")
    ...
    print(f"Evaluating Weighted Ensemble on {num_days} Days (XAUUSD Test Set)...")
    ...
    with open('RL_Agent_Final/Weighted_Ensemble_XAUUSD_Report.txt', 'w') as f:
        f.write(report)
    print("\nSaved report to RL_Agent_Final/Weighted_Ensemble_XAUUSD_Report.txt")
    ...
    # Equity Plot
    plt.figure(figsize=(10, 6))
    plt.plot(curve, label='Weighted Ensemble (XAUUSD)', color='purple')
    plt.title(f'Weighted Ensemble XAUUSD (CAGR: {((cap/initial_cap)**(365/num_days)-1)*100:.0f}%)')
    plt.yscale('log')
    plt.legend()
    plt.savefig('RL_Agent_Final/equity_curve_weighted_xauusd.png')
    print("Saved plot to RL_Agent_Final/equity_curve_weighted_xauusd.png")

if __name__ == "__main__":
    evaluate_weighted_ensemble_xauusd()
