...
def train_recurrent():
    ...
    models_dir = "RL_Agent_Final/models_recurrent"
    ...
    # Same Optimized Reward Config as train.py
    ...
    print(f"Starting Recurrent PPO Training with {N_ENVS} environments...")
    ...
    model = RecurrentPPO(
        ...
        gae_lambda=0.95,
        clip_range=0.2,
        policy_kwargs=dict(enable_critic_lstm=False) # Often faster convergence if only Actor has LSTM, or share. Default is shared.
        # Let's stick to defaults for now: shared LSTM is default in sb3-contrib RecurrentPPO
    )
    
    # Checkpoint callback
    checkpoint_callback = CheckpointCallback(save_freq=50000 // N_ENVS, save_path=models_dir, name_prefix='final_recurrent_ppo')
    ...
    # Save Final Model
    model.save(f"{models_dir}/final_model_recurrent")
    print("Recurrent Model Saved.")
    env.close()
