...
def evaluate_qrdqn():
    # Paths
    model_path = "RL_Agent_Final/models_qrdqn/final_model_qrdqn"
    
    if not os.path.exists(model_path + ".zip"):
         print(f"Model not found at {model_path}")
         return
         
    print(f"Loading QR-DQN model from {model_path}...")
    ...
    print(f"Evaluating QR-DQN on {num_days} Test Days...")
    ...
    for i in range(num_days):
        ...
        curve.append(cap)
        dr = (cap - day_start) / day_start if day_start > 0 else 0.0
    ...
    plt.plot(curve, label='QR-DQN', color='red')
    plt.title('QR-DQN Performance (Quantile Regression)')
    plt.xlabel('Days')
    plt.ylabel('Capital')
    plt.yscale('log')
    plt.legend()
    plt.savefig('RL_Agent_Final/equity_curve_qrdqn.png')
    print("Saved plot to equity_curve_qrdqn.png")
