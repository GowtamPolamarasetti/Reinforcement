...
def make_env(stacked=False):
    def _init():
        return RenkoFilterFastEnv(
            ...
            reward_config=None,
            mode='test',       # Test Mode
            split_ratio=0.0,   # 0.0 split = Full Dataset (Start from index 0)
...
def calculate_advanced_metrics(initial_cap, final_cap, trades_pnl, daily_returns, curve, days_count):
    ...
    report.append("        WEIGHTED ENSEMBLE (FULL XAUUSD)           ")
    report.append("==================================================")
    report.append(f"Metric Used: Full Dataset (Train + Test)")
...
def plot_heatmap(dates, daily_rets):
    ...
    plt.title('Monthly Returns - Weighted Ensemble (Full XAUUSD)')
    plt.tight_layout()
    plt.savefig('RL_Agent_Final/heatmap_weighted_xauusd_full.png')
    print("Saved heatmap to RL_Agent_Final/heatmap_weighted_xauusd_full.png")
    plt.close()

def evaluate_weighted_ensemble_xauusd_full():
    print("Loading Models for Weighted Ensemble (XAUUSD Full)...")
    ...
    print(f"Evaluating Weighted Ensemble on {num_days} Days (Full Dataset)...")
    ...
    with open('RL_Agent_Final/Weighted_Ensemble_XAUUSD_Full_Report.txt', 'w') as f:
        f.write(report)
    print("\nSaved report to RL_Agent_Final/Weighted_Ensemble_XAUUSD_Full_Report.txt")
    ...
    # Equity Plot
    plt.figure(figsize=(10, 6))
    plt.plot(curve, label='Weighted Ensemble (Full XAUUSD)', color='purple')
    plt.title(f'Weighted Ensemble Full XAUUSD (CAGR: {((cap/initial_cap)**(365/num_days)-1)*100:.0f}%)')
    plt.yscale('log')
    plt.legend()
    plt.savefig('RL_Agent_Final/equity_curve_weighted_xauusd_full.png')
    print("Saved plot to RL_Agent_Final/equity_curve_weighted_xauusd_full.png")

if __name__ == "__main__":
    evaluate_weighted_ensemble_xauusd_full()
